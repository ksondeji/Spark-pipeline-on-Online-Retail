...
def get_rfm_table(spark, gold_path, reference_date=None):
    ...
    rfm = (
        rfm.withColumn("r_score", quartile_score("recency_days", ascending=False))
...
def get_rfm_segment_summary(spark, gold_path):
    """Nombre de clients et CA par segment RFM — table de synthèse pour la présentation."""
    rfm = get_rfm_table(spark, gold_path)
    return (
        rfm.groupBy("rfm_segment")
           .agg(F.count("*").alias("nb_customers"),
...
def get_cohort_retention(spark, gold_path):
    ...
    activity = df.select("CustomerID", "InvoiceDate").join(first_purchase, "CustomerID")
    ...
    cohort_size = first_purchase.groupBy("cohort_month").agg(F.count("*").alias("cohort_size"))
    ...
    retention = (
        activity.groupBy("cohort_month", "period_number")
                ...
                .withColumn("retention_rate", F.round(F.col("active_customers") / F.col("cohort_size") * 100, 1))
